...
from asyncpg import Connection, Pool, connect, exceptions
    ...
    async def get_pool(self, _id) -> Pool:
        ...
        try:
            await connection.execute(
                f'CREATE DATABASE "{_id}" OWNER "{self.config.database.user}"'
            )
        except:
            pass
        finally:
            import_module("Classes.database.#main")
            self.cache["pools"][_id] = await Table.create_pool(
                self.config.database.postgresql + str(_id), **_kwargs
            )
            try:
                for table in Table.all_tables():
                    await table.create(
                        item=str(_id), verbose=False, run_migrations=False
                    )
