...
class command():
    ...
    async def __await__(self): 
        embed = ShakeEmbed.default(self.ctx, title=_("> Shake Plus (Shake+)"), description=_("{emoji} This Discord server has no premium activated.").format(emoji=self.bot.emojis.cross))
        embed.add_field(
            inline=False, name=_("{emoji} The benefit of supporting Shake as Patreon").format(emoji=self.bot.emojis.dot),
            value=_(
                ...
                """))
        embed.add_field(
            inline=False, name=_("{emoji} Helpful links for understanding Patreon").format(emoji=self.bot.emojis.dot),
            value=_(
                ...
                **[How do I connect Discord to Patreon?](https://support.patreon.com/hc/articles/212052266-How-do-I-connect-Discord-to-Patreon-Patron-)**
                """))
        embed.set_image(url="https://cdn.discordapp.com/attachments/949420891702427688/1004805116186071080/TempVoice_Premium_Banner.png")
        return await self.ctx.smart_reply(ephemeral=True, embed=embed, view=Button(self.bot))
...
class Button(ui.View):
    def __init__(self, bot):
        ...
        button = ui.Button(
                ...
                label=_("Add Shake to your server"), url=self.bot.config.other.authentication)
