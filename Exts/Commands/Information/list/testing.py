...
class PageSource(ListPageSource):
    def add_field(self, embed, item):
        tick = str(PartialEmoji(name='left', id=1033551843210579988)) if self.kwargs.get('current', MISSING) == item else ''
        embed.add_field(
            name='` {index}. ` \N{EM DASH} {name} {tick}'.format(index=self.items.index(item)+1, name=item.name, tick=tick), 
            value=_("**ID**: `{id}` **OWNER**: `{owner}` **MEMBERS**: `{members}`").format(id=item.id, owner=item.owner, members=len(item.members)), inline=False
        )
