...
class command():
    def __init__(self, ctx: ShakeContext, user: User, member: Optional[Member] = None, fallback: Optional[Member] = None):
    ...
    async def __await__(self):
        ...
        menu = Menu(ctx=self.ctx, source=Front(), user=self.user, member=self.member, fallback=self.fallback, select=select)
        ...
        if self.member:
            member = {
                ...
                PositionSource(ctx=self.ctx, member=self.member): list(self.member.guild.members),
            }
...
class Menu(CategoricalMenu):
    def __init__(
            self, ctx: ShakeContext, source: menus.PageSource, user: User, member: Optional[Member] = None, fallback: Optional[Member] = None,
    ...
    def embed(self, embed: ShakeEmbed):
        ...
        embed.set_author(name=self.user.name, icon_url=getattr(self.user.avatar, 'url', recovery))
        return embed


class RolesSource(ListPageSource):
    guild: Guild
    def __init__(self, ctx: Union[ShakeContext, Interaction], member: Member, *args, **kwargs):
        self.member: Member = member
        self.guild: Guild = member.guild
        roles: List[Role] = sorted([role for role in member.roles if not role == member.guild.default_role], reverse=True)
...
class AssetsSource(ListPageSource):
    guild: Guild
    def __init__(self, ctx: Union[ShakeContext, Interaction], user: User, member: Optional[Member] = None, *args, **kwargs):
        self.user: User = user
        self.member: Optional[Member] = member
        self.from_dict: Dict[Asset, str] = {
            member.avatar if member else user.avatar: _("User's Avatar"), member.display_avatar if member else getattr(user, 'display_icon', None): _("User's Display Avatar"),
            member.guild_avatar if member else None: _("User's Guild Avatar"), user.banner: _("User's Banner")
        }
...
class PositionSource(ItemPageSource):
    ...
    def __init__(self, ctx: Union[ShakeContext, Interaction], member: Member, *args, **kwargs):
        ...
        self.guild: Guild = member.guild
        super().__init__(
            ctx, item=sorted(self.guild.members, key=lambda member: sum(m.joined_at < member.joined_at for m in self.guild.members)), 
            title=MISSING, label=_("Joining Postition"), paginating=True, per_page=25, *args, **kwargs
        )
...
class BadgesSource(ItemPageSource):
    user: User
    def __init__(self, ctx: Union[ShakeContext, Interaction], user: User, *args, **kwargs):
        self.user: User = user
        super().__init__(ctx, item=user.public_flags, title=MISSING, label=_("Badges"), paginating=True, per_page=25, *args, **kwargs)
    ...
    def format_page(self, menu: Menu, *args: Any, **kwargs: Any) -> ShakeEmbed:
        ...
        for property in set([flag for flag, has in flags if has]):
            ...
            name = ' '.join(list(x.capitalize() for x in str(property).replace('_', ' ').split()))
            embed.add_field(
                name=str(badge) +' '+ name, value='\u200b'
            )
