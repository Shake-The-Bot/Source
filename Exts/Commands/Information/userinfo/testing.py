...
from Classes.useful import MISSING, FormatTypes, TextFormat
...
class RolesSource(ListPageSource):
    ...
    def __init__(
        self, ctx: Union[ShakeContext, Interaction], member: Member, *args, **kwargs
    ):
...
class AssetsSource(ListPageSource):
    ...
    def format_page(self, menu: Menu, items: Asset, **kwargs: Any) -> ShakeEmbed:
        ...
        formattypes = list(
            get_args(
                ValidAssetFormatTypes if items.is_animated() else ValidStaticFormatTypes
            )
        )
...
class PositionSource(ItemPageSource):
    ...
    def format_page(self, menu: Menu, *args: Any, **kwargs: Any) -> ShakeEmbed:
        ...
        embed.add_field(
            name=_("Joined on"),
            value=q(b(format_dt(self.member.joined_at, style="F"))),
        )
...
class BadgesSource(ItemPageSource):
    ...
    def __init__(
        self, ctx: Union[ShakeContext, Interaction], user: User, *args, **kwargs
    ):
        self.user: User = user
        super().__init__(
            ctx,
    ...
    def format_page(self, menu: Menu, items: Any):
        ...
        if m := member or fallback:
            ...
            if member:
                ...
                embed.add_field(
                    name=_("Server Owner"), value=q(tick[member == member.guild.owner])
                )
        ...
        embed.add_field(
            ...
            value=mq(
                "\n".join(
                    b(str(k) + str(s) + " " + str(v)) for (k, s), v in more.items()
                )
            ),
            inline=False,
        )
        ...
        if banner := getattr(user, "banner", None):
            ...
            embed.add_field(
                name="\u200b", value=q(menu.bot.config.embed.footer), inline=False
            )
        return embed, None
...
class ActivitieSource(ItemPageSource):
    ...
    def __init__(
        self, ctx: Union[ShakeContext, Interaction], user: Member, *args, **kwargs
    ):
