...
from discord.activity import ActivityType, ActivityTypes
...
from Classes.useful import MISSING, FormatTypes, TextFormat, human_join
...
class RolesSource(ListPageSource):
    ...
    def __init__(
        self, ctx: Union[ShakeContext, Interaction], guild: Guild, *args, **kwargs
    ):
        self.guild: Guild = guild
...
class ChannelsSource(ListPageSource):
    ...
    def format_page(
        self, menu: Menu, items: List[GuildChannel], **kwargs: Any
    ) -> ShakeEmbed:
...
class MembersSource(ItemPageSource):
    ...
    def format_page(self, menu: Menu, *args: Any, **kwargs: Any) -> ShakeEmbed:
        ...
        infos = {
            ...
            str(human)
            + " "
            + _("Total Humans"): multic(len([m for m in members if not m.bot])),
            str(bot)
            + " "
            + _("Total Bots"): multic(len([m for m in members if m.bot])),
            str(emojis.online)
            + " "
            + _("Total Online"): multic(
                len([m for m in members if m.status == Status.online])
            ),
            str(emojis.online)
            + " "
            + _("Humans Online"): multic(
                len([m for m in members if not m.bot and m.status == Status.online])
            ),
            str(emojis.online)
            + " "
            + _("Bots Online"): multic(
...
class Front(FrontPageSource):
    def format_page(self, menu: Menu, items: Any):
        ...
        embed.add_field(name=_("Region"), value=q(result), inline=False)
        ...
        statuses = "︱".join(
            [
                str(emojis.online) + c(status["online"]),
                str(emojis.idle) + c(status["idle"]),
                str(emojis.dnd) + c(status["dnd"]),
                str(emojis.offline) + c(status["offline"]),
            ]
        )
        embed.add_field(
            name=_("Members"),
            value=mq(
                f'{b(len(set(m for m in guild.members if not m.bot)))} (+{bots} {_("Bots")})\n{statuses}'
            ),
            inline=False,
        )

        embed.add_field(
            name=_("Created"), value=q(format_dt(guild.created_at, style="F"))
        )
        more: Dict[str, str] = {
            _("ID"): f"`{guild.id}`",
