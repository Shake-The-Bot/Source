...
from Classes import ShakeContext, ShakeEmbed, DurationDelta, ShakeBot
########
#
class command():
    ...
    async def __await__(self):
        ...
        try:
            for member in self.member: 
                await member.timeout(until=timedelta(seconds=self.time), reason='{reason} {author}'.format(reason=self.reason, author=self.ctx.author))

        except Exception as error:
            raise error
        
        else:
            embed = ShakeEmbed.to_success(self.ctx, description=_("The specified members were successfully muted"))
