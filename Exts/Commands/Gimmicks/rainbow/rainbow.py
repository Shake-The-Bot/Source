...
class command():
    ...
    async def __await__(self):
        ...
        embed = ShakeEmbed.default(self.ctx, description = _(
            "**{emoji} {prefix} `{user}` is part of it**").format(
                emoji=self.bot.emojis.hook, prefix=self.bot.emojis.prefix, user=self.member
        ))
        embed.set_image(url=f'attachment://{name}.png')
        await self.ctx.smart_reply(embed=embed, file=file)
