...
from .wouldyous import useful, useless
########
#
class command():
    ...
    async def __await__(self, _useful: bool = False):
        ...
        if self.rather:
            view = RatherView(self.ctx, _useful, self.voting, key)
        else:
            view = VoteView(self.ctx, _useful, self.voting, key)
...
class VoteView(ui.View):
    def __init__(self, ctx, _useful, voting, key):
        super().__init__(timeout=20)
        self.ctx: ShakeContext = ctx
        self.bot: ShakeBot = ctx.bot
        self.key: str = key
        self.voting = voting
        self._useful = _useful
        self.no: list = list()
        self.yes: list = list()
...
class RatherView(ui.View):
    def __init__(self, ctx: ShakeContext, _useful, voting, key):
        super().__init__(timeout=20)
        self.ctx: ShakeContext = ctx
        self.bot: ShakeBot = ctx.bot
        self.key: str = key
