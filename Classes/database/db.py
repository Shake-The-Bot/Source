...
class Table(metaclass=TableMeta):  # type: ignore  # Pyright Bug I think
    ...
    @classmethod
    def write_migration(cls, item, *, directory='Migrations'):
        ...
        directory = Path(directory) / item / cls.__tablename__
    ...
    @classmethod
    async def migrate(cls, item, *, directory='migrations', index=-1, downgrade=False, verbose=False, connection=None):
    ...
    @classmethod
    async def drop(cls, item, *, directory='Migrations', verbose=False, connection=None):
...
async def _create_pool(config) -> asyncpg.Pool:
    ...
    return {
        'bot': await asyncpg.create_pool(config.database.postgresql+'bot', init=init, **_kwargs), 
        'config': await asyncpg.create_pool(config.database.postgresql+'config', init=init, **_kwargs)
    }
