...
class Event:
    ...
    def embed_from_product(self, game: ProductDataType):
        ...
        if (launcher := getattr(game, "launcher", None)) and launcher is not None:
            embed.add_field(
                name=_("Or open directly in launcher"),
                value="**{launcher}**".format(launcher=launcher),
            )
    ...
    async def __await__(self):
        ...
        stores: dict = {
            store: {game: self.embed_from_product(game)}
            for game in self.games
            if game.store == store
            for store in set(game.store for game in self.games)
        }

        for row in (
            config.fetch("SELECT * FROM freegames WHERE guild_id = $1", self.guild.id)
            or []
        ):
            if not (channel := self.bot.get_channel(row.get("channel_id"))):
                config = await self.bot.config_pool
                config.execute(
                    "DELETE FROM freegames WHERE channel_id = $1", row.get("channel_id")
                )
                continue

            for embeds in self.divide_chunks(
                [
                    stores[store][game]
                    for store in row.get("stores")
                    for game in stores[store]
                ],
                10,
            ):
