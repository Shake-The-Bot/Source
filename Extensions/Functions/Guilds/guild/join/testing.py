    ...
    async def __await__(self):
        ...
        def introdoucing():
            with suppress(AttributeError):
                ...
                for chat in self.guild.text_channels:
                    if (
                        chat.permissions_for(self.guild.default_role).send_messages
                        and chat.permissions_for(self.guild.default_role).read_messages
                    ):
                        return chat
                for chat in self.guild.text_channels:
        ...
        embed.set_author(
            ...
            icon_url=PartialEmoji(
                animated=True, name="thanks", id=984980175525666887
            ).url,
        )
        embed.description = """
