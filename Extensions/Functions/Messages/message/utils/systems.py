...
class aboveme:
    ...
    async def createrecord(self, db):
        await db.execute(
            "INSERT INTO aboveme (channel_id) VALUES ($1) ON CONFLICT (channel_id) DO NOTHING;",
            self.channel.id,
        )
    ...
    async def __await__(self):
        ...
        record = await db.fetchrow(
            "SELECT * FROM aboveme WHERE channel_id = $1", self.channel.id
        )
            ...
            if config["hardcore"]:
                embed.description = _(
                    ...
                    Someone should still go on.""".format(
                        ...
                        count=record["count"],
                        facepalm=str(
                            PartialEmoji(name="facepalm", id=1038177759983304784)
                        ),
                    )
                )
        ...
        if (self.message.content.isdigit()) or (
            not self.message.content.lower().startswith(_("the one above me").lower())
        ):
            embed = ShakeEmbed(
                description=_(
                    "{emoji} Your message should start with **{trigger}**"
                ).format(
                    emoji="<a:nananaa:1038185829631266981>",
                    trigger=_("the one above me"),
                )
            )
            self.kwargs["embeds"].append(embed)
            self.do["add_bad_reaction"] = True
        ...
        if self.message.content in record.get("phrases", []):
            embed = ShakeEmbed(
                description=_("{emoji} Your message should be something new").format(
                    emoji="<a:nananaa:1038185829631266981>",
                )
            )
            self.kwargs["embeds"].append(embed)
        ...
        async with db.acquire():
            ...
            if len(phrases) >= 10:
                phrases.pop(-1)
                phrases.insert(0, self.message.content)
            await db.execute(
...
class oneword:
    ...
    async def createrecord(self, db):
        async with db.acquire():
            await db.execute(
                "INSERT INTO oneword (channel_id) VALUES ($1) ON CONFLICT (channel_id) DO NOTHING;",
                self.channel.id,
            )
    ...
    async def __await__(self):
        ...
        record = await db.fetchrow(
            "SELECT * FROM oneword WHERE channel_id = $1", self.channel.id
        )
        ...
        # nur ein wort
        if (
            (self.message.content.isdigit())
            or (not len(self.message.content.split()) == 1)
            or (
                not self.message.content.startswith((".", "!", "?"))
                and self.message.content.endswith((".", "!", "?"))
            )
        ):
            embed = ShakeEmbed(
                description=_(
                    "{emoji} your message should contain **only one** word"
                ).format(
                    emoji="<a:nananaa:1038185829631266981>",
                    trigger=_("the one above me"),
                )
...
class counting:
    ...
    async def createrecord(self, db):
        async with db.acquire():
            await db.execute(
                "INSERT INTO counting (channel_id) VALUES ($1) ON CONFLICT (channel_id) DO NOTHING;",
                self.channel.id,
            )
    ...
    async def __await__(self):
        ...
        record = await db.fetchrow(
            "SELECT * FROM counting WHERE channel_id = $1", self.channel.id
        )
        ...
        if (not self.message.content.isdigit()) and (not config["numbersonly"]):
            embed = ShakeEmbed(
                description=_(
                    "{emoji} You're not allowed to use anything except numbers here".format(
                        emoji="<a:nananaa:1038185829631266981>"
                    )
                )
            )
        ...
        if not int(self.message.content) == int(record["count"]):
            ...
            embed = ShakeEmbed(
                description=_(
                    """{user} ruined it at **{count}** {facepalm}. **You apparently can't count properly**. The __next__ number {verb} ` {last} `. {streak}"""
                ).format(
                    user=self.member.mention,
                    count=record["count"],
                    facepalm="<:facepalm:1038177759983304784>",
                    streak=_(
                        "**You've topped your best streak with {} 🔥**".format(
                            self.streak
                        )
                    )
                    if self.streak > self.best_streak
                    else "",
                    verb=(_("is") if not last_ten == record["count"] else _("remains")),
                    last=last_ten,
                    number=int(self.message.content),
                )
            ...
            async with db.acquire():
                await db.execute(
                    ...
                    self.member.id,
                    last_ten,
                    self.streak if self.streak > self.best_streak else self.best_streak,
                )
            self.do["add_bad_reaction"] = True if int(record["count"]) != 1 else False
            self.do["add_warning_reaction"] = (
                True if int(record["count"]) == 1 else False
            )
            return
        ...
        if config["goal"] and int(self.message.content) >= config["goal"]:
            ...
            await self.bot.config_pool.execute(
                "UPDATE counting SET goal = $2 WHERE channel_id = $1",
                record["channel_id"],
                None,
            )
