...
from Classes import AboveMeBatch, CountingBatch, ShakeBot, Testing
...
class on_message(Cog):
    def __init__(self, bot: ShakeBot):
        ...
        self.counting_bulk_update.add_exception_type(PostgresConnectionError)
        self.counting_bulk_update.start()
        self.aboveme_bulk_update.add_exception_type(PostgresConnectionError)
        self.aboveme_bulk_update.start()

    async def cog_unload(self):
        await self.counting_bulk_update()
        self.counting_bulk_update.stop()
        await self.aboveme_bulk_update()
        self.aboveme_bulk_update.stop()

    @loop(seconds=60.0)
    async def counting_bulk_update(self):
        query = """UPDATE counting set user_id = x.user_id, count = x.count, streak = x.streak, used = x.used, best = x.best, goal = x.goal 
                   FROM jsonb_to_recordset($1::jsonb) AS x(channel_id BIGINT, count BIGINT, user_id BIGINT, streak BIGINT, best BIGINT, goal BIGINT, used TIMESTAMP, number BOOLEAN) 
    ...
    @loop(seconds=60.0)
    async def aboveme_bulk_update(self):
        query = """UPDATE aboveme set user_id = x.user_id, count = x.count, phrases = x.phrases, used = x.used
                   FROM jsonb_to_recordset($1::jsonb) AS x(channel_id BIGINT, user_id BIGINT, count BIGINT, phrases TEXT[], used TIMESTAMP) 
