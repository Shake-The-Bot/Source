...
class TempVoice:
    ...
    async def __await__(self, interaction):
        ...
        record = (
            await self.db.fetchrow(
                "SELECT * FROM tempvoice WHERE user_id = $1", self.member_id
            )
            or None
        )
    ...
    async def rename(self):
        ...
        class request(ui.Modal):
            name = ui.TextInput(
                label=_("CHOOSE A NAME FOR YOUR VOICE CHANNEL"),
                placeholder=_("Leave blank to reset the name"),
                custom_id="tempvoice-rename-request-name",
                max_length=100,
                required=False,
            )
            ...
            def __init__(self, tempvoice: TempVoice) -> None:
                super().__init__(
                    title="TEMPVOICE",
                    timeout=None,
                    custom_id="tempvoice-rename-request",
                )
            ...
            async def on_submit(self, interaction: Interaction):
                ...
                record = await self.db.fetchrow(
                    "SELECT * FROM tempvoice WHERE user_id = $1", self.member.id
                )  # old
                config = await self.bot.config_pool.fetchrow(
                    "SELECT * FROM tempvoice WHERE creator_id = $1",
                    record["creator_id"],
                )
                ...
                await channel.edit(
                    name=name, reason="TempVoice - executed by {}".format(self.member)
                )
    ...
    async def limit(self):
        ...
        class request(ui.Modal):
            limit = ui.TextInput(
                label=_("CHOOSE A LIMIT FOR YOUR VOICE CHANNEL"),
                required=False,
                placeholder=_("Leave blank to reset the limit"),
                max_length=100,
            )
            ...
            async def on_submit(self, interaction: Interaction):
                ...
                await channel.edit(
                    user_limit=self.limit if not self.limit == 0 else None,
    ...
    async def privacy(self):
        ...
        class request(ui.Select):
            def __init__(self, tempvoice: TempVoice, options):
                super().__init__(
                    placeholder=_("Select a Privacy Option"),
                    min_values=1,
                    max_values=1,
                    options=options,
                )
            ...
            async def callback(self, interaction: Interaction):
                str_values = {
                    "tempvoice-privacy-request-unlocked": _("unlocked"),
                    "tempvoice-privacy-request-locked": _("locked"),
                    "tempvoice-privacy-request-invisible": _("invisible"),
                    "tempvoice-privacy-request-visible": _("visible"),
                }
                value = self.values[0]
                ...
                connect = (
                    (True if value == "tempvoice-privacy-request-unlocked" else False)
                    if value
                    in [
                        "tempvoice-privacy-request-locked",
                        "tempvoice-privacy-request-unlocked",
                    ]
                    else overwrite.connect
                )
                view_channel = (
                    (True if value == "tempvoice-privacy-request-visible" else False)
                    if value
                    in [
                        "tempvoice-privacy-request-invisible",
                        "tempvoice-privacy-request-visible",
                    ]
                    else overwrite.view_channel
                )
                ...
                await channel.set_permissions(
                    ...
                    use_embedded_activities=overwrite.use_embedded_activities,
                    reason="TempVoice - executed by {}".format(self.member),
                )
        ...
        class View(ui.View):
            ...
            def get_options(self):
                ...
                invisible = {
                    "request": overwrite.view_channel == True,
                    "emoji": "<:invisible:1047200587931975832>",
                    "label": _("Invisible"),
                    "description": _(
                        "Everyone will be able to view your voice channel"
                    ),
                    "value": "tempvoice-privacy-request-invisible",
                }
                visible = {
                    "request": not invisible.get("request"),
                    "emoji": "<:visible:1047200589668438027>",
                    "label": _("Visible"),
                    "description": _(
                        "Only trusted users will be able to view your voice channel"
                    ),
                    "value": "tempvoice-privacy-request-visible",
                }
                locked = {
                    "request": True
                    if not invisible.get("request", True)
                    else overwrite.connect == True,
                    "emoji": "<:locked:1047200592604434533>",
                    "label": _("Lock"),
                    "description": _(
                        "Only trusted users will be able to join your voice channel"
                    ),
                    "value": "tempvoice-privacy-request-locked",
                }
                unlocked = {
                    "request": not locked.get("request"),
                    "emoji": "<:unlocked:1047200591392276530>",
                    "label": _("Unlock"),
                    "description": _(
                        "Everyone will be able to join your voice channel"
                    ),
                    "value": "tempvoice-privacy-request-unlocked",
                }
                    ...
                    if values.get("request", False) != False:
                        self.options.append(
                            SelectOption(
                                ...
                                value=values.get("value", utils.MISSING),
    ...
    async def transfer(
        ...
        class View(ui.View):
            ...
            async def request(self, interaction: Interaction, select: ui.UserSelect):
                ...
                record: Dict = await self.db.fetchrow(
                    "SELECT * FROM tempvoice WHERE user_id = $1", self.member.id
                )
                config = await self.bot.config_pool.fetchrow(
                    "SELECT * FROM tempvoice WHERE creator_id = $1",
                ...
                await self.db.execute(
                    "UPDATE tempvoice SET user_id = $2 WHERE channel_id = $1",
                    channel.id,
                    user.id,
                )
                await self.old_interaction.edit_original_response(
    ...
    async def trust(self):
        ...
        class View(ui.View):
            ...
            @ui.select(
                cls=ui.UserSelect,
                placeholder=_("Selected users will be trusted to join!"),
                min_values=1,
                max_values=25,
            )
            async def request(self, interaction: Interaction, select: ui.UserSelect):
                ...
                for overwrite in [
                    overwrites := {
                        user: PermissionOverwrite(view_channel=True, connect=True)
                        for user in users
                    }
                ]:
                    await channel.set_permissions(
                ...
                await self.db.execute(
                    "UPDATE tempvoice SET trusted = $2, blocked = $3 WHERE channel_id = $1",
                    channel.id,
                    trusted,
                    blocked,
                )
    ...
    async def block(self):
        ...
        class View(ui.View):
            ...
            @ui.select(
                cls=ui.UserSelect,
                placeholder=_("Selected users will be kicked and blocked!"),
                min_values=1,
                max_values=25,
            )
            async def request(self, interaction: Interaction, select: ui.UserSelect):
                ...
                for user in [
                    overwrites := {
                        user: PermissionOverwrite(view_channel=False) for user in users
                    }
                ]:
                    await channel.set_permissions(
                        target=user,
                        overwrite=overwrites[user],
                        reason="TempVoice - executed by {}".format(self.member),
                    )
                ...
                await self.db.execute(
                    "UPDATE tempvoice SET blocked = $2, trusted = $3 WHERE channel_id = $1",
                    channel.id,
                    blocked,
                    trusted,
                )
    ...
    async def kick(self):
        ...
        class View(ui.View):
            ...
            @ui.select(
                cls=ui.UserSelect,
                placeholder=_(
                    "Selected user will be kicked from your tempvoice channel!"
                ),
                min_values=1,
                max_values=25,
            )
            async def request(self, interaction: Interaction, select: ui.UserSelect):
                ...
                for user in users:
                    if (
                        not (user.voice)
                        or not (user.voice.channel == channel)
                        or (user == self.member)
                    ):
                        with suppress(ValueError):
                            users.remove(user)
                ...
                for user in users:
                    await user.move_to(
                        None, reason="TempVoice - executed by {}".format(self.member)
                    )
    ...
    async def claim(self):
        ...
        records = await self.db.fetch("SELECT * FROM tempvoice")
        record: Dict = await self.db.fetchrow(
            "SELECT * FROM tempvoice WHERE channel_id = $1", channel.id
        )
        config = await self.bot.config_pool.fetchrow(
            "SELECT * FROM tempvoice WHERE creator_id = $1", record["creator_id"]
        )
        old_owner = self.guild.get_member(record["user_id"])
        if old_owner == self.member or (
            self.member.id in set(record["user_id"] for record in records)
        ):
            await self.interaction.response.send_message(
                ephemeral=True,
                content=_(
    ...
    async def untrust(self):
        ...
        class View(ui.View):
            ...
            @ui.select(
                cls=ui.UserSelect,
                placeholder=_("Selected users will be untrusted from joining!"),
                min_values=1,
                max_values=None,
            )
            async def request(self, interaction: Interaction, select: ui.UserSelect):
                ...
                for overwrite in [
                    overwrites := {
                        user: PermissionOverwrite(view_channel=None, connect=None)
                ...
                await self.db.execute(
                    "UPDATE tempvoice SET trusted = $2 WHERE channel_id = $2",
                    channel.id,
                    trusted,
                )
                await self.old_interaction.edit_original_response(
    ...
    async def unblock(self):
        ...
        class View(ui.View):
            ...
            @ui.select(
                cls=ui.UserSelect,
                placeholder=_("Selected users will be unblocked from joining!"),
                min_values=1,
                max_values=None,
            )
            async def request(self, interaction: Interaction, select: ui.UserSelect):
                ...
                await self.db.execute(
                    "UPDATE tempvoice SET blocked = $2 WHERE channel_id = $2",
                    channel.id,
                    blocked,
                )
                await self.old_interaction.edit_original_response(
