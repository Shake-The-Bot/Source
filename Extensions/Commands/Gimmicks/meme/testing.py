...
class command:
    ...
    def format(self, post: Submission):
        preview = (
            post.__dict__["preview"]["images"][0]
            if bool(len(post.__dict__["preview"]["images"]))
            else None
        )
        ...
        embed.set_author(
            icon_url=PartialEmoji(
                animated=True, name="meme", id=952690616754655293
            ).url,
            name=_("„{title}“ by {author}").format(
                title=str(post.title), author=post.author.name
            ),
        )
        embed.description = (
            "*"
            + _("Like this meme? Check out others at \n{subreddit}").format(
    ...
    async def __await__(self):
        ...
        embed = None
        while embed is None:
            post: Submission = await self.bot.reddit.get_post(
                ctx=self.ctx, subreddit=self.subreddit
            )
