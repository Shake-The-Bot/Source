...
class command(ShakeCommand):
    ...
    async def __await__(self):
        ...
        if self.rather:
            embed.add_field(
                name="Would you rather have this",
                value="> :one: {}".format(q1),
                inline=False,
            )
            embed.add_field(
                name="or this {utility} {key}?".format(utility=self.utility, key=key),
                value="> :two: {}".format(q2),
                inline=False,
            )
        else:
            embed.add_field(
                name="Would you want this {utility} {key}?".format(
                    utility=self.utility, key=key
                ),
                value=">>> {}".format(q1),
                inline=False,
            )
...
class VoteView(ui.View):
    ...
    async def set_stats(self, message):
        ...
        self.stats: Optional[bool] = (
            None if (self.positive == self.negative) else self.positive > 50
        )
    ...
    async def on_timeout(self) -> None:
        if self.voting:
            ...
            if self.stats == True:
                value = (
                    "{emoji} {yes} of {total} user(s) ({procent}) would take this {key}"
                )
            ...
            procent = (
                str(self.positive if self.stats == True else self.negative) + "%"
                if not self.tie
            ...
            self.embed.colour = (
                (Colour.red() if self.stats == False else Colour.green())
                if not self.tie
    ...
    @ui.button(
        emoji=PartialEmoji(name="cross", id=1092840626787197060),
        style=ButtonStyle.blurple,
        row=1,
    )
...
class RatherView(ui.View):
    ...
    async def set_stats(self, message):
        ...
        self.procent = str(
            self.percentage1
            if self.percentage1 >= self.percentage2
            else self.percentage2
        )
        self.power = 1 if self.percentage1 >= self.percentage2 else 2
        self.users = (
            len(self.u1) if self.percentage1 >= self.percentage2 else len(self.u2)
        )
    ...
    async def on_timeout(self) -> None:
        if self.voting:
            await self.set_stats(self.message)
            emoji = (
                ("1️⃣" if self.power == 1 else "2️⃣")
                if self.total != 0
                else "<:blobsad:1056292826574504066>"
            )
            self.embed.insert_field_at(
                name="Results",
                inline=False,
            ...
            self.embed.colour = (
                Colour.og_blurple() if self.total != 0 else Colour.light_gray()
            )
            await self.message.edit(embed=self.embed, view=None)
        return await super().on_timeout()
